...
UpSampling2D, BatchNormalization, LeakyReLU, Activation, AveragePooling2D, MaxPooling2D, subtract, GlobalMaxPooling2D
...
def G(in_w = (w_hei, w_wid, 1), in_c = (height, width, 3), scale=1):
    ...
    W_code_mean = Dense(z_dim)(x)
    W_code_var = Dense(z_dim)(x)
    
    G1_model = Model(inputs=[C,W], outputs=M, name='G1')
    G2_model = Model(inputs = [C,W],outputs = [W_code_mean, W_code_var], name = 'G2' )
...
def D2(in_shape = (z_dim * 2,)):

    z_in = Input(shape = in_shape)
    z = z_in
    ...
    D2 = Model(inputs = z_in, outputs = z)
    D2.summary()
...
def StegoGAN(in_w=(w_hei, w_wid, 1), in_c=(height, width, 3)):
    ...
    C = Input(shape=in_c, name='C')
    W = Input(shape=in_w, name='W')
    G1_model.trainable = False
    #G2_model.trainable = False
    ...
    G1_model.trainable = True
    D1_model.trainable = False

    x1_fake = G1_model([C,W])
    ...
    W_mean, W_var = G2_model([C,W])

    z_samples = Lambda(sampling)([W_mean, W_var])
    prior_kl_loss = -0.5*K.mean(1+ W_var - K.square(W_mean) - K.exp(W_var))

    #  c. G connected to D2
    z_shuffle = Lambda(shuffling)(z_samples)
    z_true = concatenate([z_samples, z_samples],axis = -1)
    z_false = concatenate([z_samples, z_shuffle],axis = -1)

    z_true_score = D2(z_true)
    z_false_score = D2(z_false)
   
    info_loss = - K.mean(K.log(z_true_score + 1e-9) + K.log(1 - z_false_score + 1e-9))
    GD2_model = Model(inputs=[C, W], outputs=[z_true_score, z_false_score])
    GD2_model.add_loss(info_loss + prior_kl_loss)
    ...
    return d1_train_model, G_train_model, GD2_model, G1_model

    
def train(epochs = 100):
    d1_train_model, G_train_model, GD2_model, G1_model = StegoGAN()
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps):
            ...
            for j in range(1):
                ...
                info_loss = GD2_model.train_on_batch([C,W],[])
            ...
            print('\r', step,'/',steps-1, 'D_loss: ', str(D_loss).ljust(10), \
                ...
                'info_loss:', str(D2_loss).ljust(10),\
                end='')
