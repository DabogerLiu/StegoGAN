...
dataset_len = 10500/2  # total images
...
UpSampling2D, BatchNormalization, LeakyReLU, Activation, AveragePooling2D, MaxPooling2D, subtract
...
def To_latent(in_m = (height, width, 3)):
...
def Q(M_shape=(height, width, 3), W_shape=(w_hei, w_wid, 1)):
    I1 = Input(shape = M_shape, name='D2_M_shape')
    I2 = Input(shape = W_shape, name='D2_W_shape')
    I2_3 = concatenate([I2,I2,I2],axis=-1)

    latent_M = To_latent(I1) #Generator output
    latent_W = To_latent(I2_3) # Real image

    model = Model(inputs=[I1,I2], outputs=[latent_M,latent_W])
    model.compile(loss= mutual_info_loss, optimizer = Adam(0.0002, 0.5) )
    model.summary()
...
def G_D1_model(in_w=(w_hei, w_wid, 1), in_c=(height, width, 3)):
    ...
    latent_M = To_latent(M) #Generator output
    latent_W = To_latent(W) # Real image
    Qmodel = Model(inputs=[C,W], outputs=[latent_M,latent_W])
    Qmodel.compile(loss= mutual_info_loss, optimizer = Adam(0.0002, 0.5) )
    ...
    return d1_train_model, G_train_model,GD2_model ,G_model, D1_model, Qmodel




def GD2_model(in_w=(w_hei, w_wid, 1), in_c=(height, width, 3)):
    G_model = G()
    #R_model = R()
    D2_model = D2()
    C = Input(shape=in_c, name='C')
    W = Input(shape=in_w, name='W') #shuffle W
    M = G_model([C,W]) # Encode:  C + W --> M

    
    W_shuffle = Lambda(shuffling)(W)

    score2_t = D2_model([M,W]) #True score
    score2_f = D2_model([M, W_shuffle]) #False score
    dist = K.mean(score2_t - score2_f)

    d2_loss = - K.mean(K.log(score2_t + 1e-9) + K.log(1 - score2_f + 1e-9))
    GD2_model = Model(inputs=[C, W], outputs=[score2_t, score2_f])
    GD2_model.add_loss(d2_loss)
    #GD2_model.metrics_names.append('dist')
    #GD2_model.metrics_tensors.append(dist)
    GD2_model.compile(optimizer='adam')

    print("===========================")
    print("Model  GD2:CW->M, MW->D2")
    GD2_model.summary()
...
def train(epochs = 100):
    d1_train_model, G_train_model, GD2_model,G_model, D1_model, Qmodel = G_D1_model()
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps):
            ...
            for j in range(5):
                D1_loss = d1_train_model.train_on_batch([C,W],[])
            for j in range(5):
                D2_loss = GD2_model.train_on_batch([C,W],[])
            for j in range(1):
                G_loss = G_train_model.train_on_batch([C,W],[])
            for j in range(1): 
                Q_loss = Qmodel.train_on_batch([C,W],[])
            
            
            print('\r', step,'/',steps-1, 'D1_loss: ', str(D1_loss).ljust(10), \
                'G_loss: ', str(G_loss).ljust(10),\
                'D2_loss', str(D2_loss).ljust(10),\
                'Q_loss', str(Q_loss).ljust(10),\
                end='')
